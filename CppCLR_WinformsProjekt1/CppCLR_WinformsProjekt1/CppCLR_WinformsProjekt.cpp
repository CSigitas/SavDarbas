...
void fillVector(vector<student>& newClass, const string& fileName) {
    ...
    if (file.is_open()) {
        while (getline(file, feilute)) {
            c++;
            if (c == 1) {
                MyFile1 << "Vardas         Pavarde        Galutinis (Vid.)  /  Galutinis (Med.)" << endl;
            }
            else {
                stringstream ss(feilute);
                while (ss >> fzodziai) {
...
void random(int g, const string& file) {
    ...
    for (int i = 0; i < 10;i++) {
        if (i == 9)
            MyFile << "ND" << to_string( i + 1 )<< endl;
        else {
            MyFile << "ND" << to_string(i + 1) << " ";
        }
    }
    for (int i = 0; i < g;i++) {
        ...
        for (int i = 0; i < 10;i++) {
            if (i == 9) {
                MyFile << left << setw(4) << to_string(rand() % 10 + 1) << endl;
            }
            else {
                MyFile << left << setw(4) << to_string(rand() % 10 + 1);
            }
        }
    }
}
